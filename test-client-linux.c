...
#include <string.h>
int main() {
    ccn_client_socket* client_socket = ccn_client_open_socket(CCN_AF_IP4, CCN_STREAM_SOCKET, 0);
    ...
    int32_t result = ccn_client_connect(client_socket, server_ip_address, server_port);
    if (result == -1) {
    ...
    char* message = fgets((char*)client_socket->buffer, CCN_BUFFER_CAPACITY, stdin);
